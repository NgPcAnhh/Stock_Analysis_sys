...
const SESSION_STORAGE_KEY = "finvision:copilot:session-id";
const HISTORY_STORAGE_KEY = "finvision:copilot:history";
...
export default function CopilotPage() {
    ...
    const askQuestion = async (value: string) => {
        ...
        try {
            const response = await fetchWithAuth(`${API}/copilot/query`, {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                const detail = typeof payload?.detail === "string" ? payload.detail : "Khong the goi copilot";
                throw new Error(detail);
            }

            const data = payload as CopilotResponse;
            ...
            <div className="mx-auto w-full max-w-3xl px-4 pb-6 mt-auto">
                <form onSubmit={onSubmit} className="relative flex flex-col rounded-2xl border border-border bg-background shadow-sm focus-within:ring-2 focus-within:ring-primary/20">
                    <textarea
                        ...
                        rows={1}
                        placeholder="Hoi AI ve tai chinh..."
